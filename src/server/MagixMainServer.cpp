				...
				case ID_PVP:
					{
						...
						tReceiveBit.Read(tMessage);
						stringCompressor->DecodeString(tUsername,16,&tReceiveBit);
						stringCompressor->DecodeString(tPlayer,16,&tReceiveBit);

						char tPlayerAccess[16] = "";
						bool tPvpAccess = false;
						std::ifstream inFile(getFilename(tPlayer,".pvp").c_str());
						if(!inFile.good())
						{
							inFile.clear();
							inFile.open(getFilename(tPlayer,".pvp",false).c_str(),std::ios::binary);
							rename(getFilename(tPlayer,".pvp",false).c_str(),getFilename(tPlayer,".pvp").c_str());
						}
						if(inFile.good())
						{
							while(!inFile.eof() && inFile.good())
							{
								inFile.getline(tPlayerAccess,16);
								if(tPlayerAccess==tUsername)
								{
									tPvpAccess = true;
									break;
								}
								...
								{
									std::ofstream outFile(getFilename(tUsername,".pvp").c_str());
									tPvpAccess = false;
									outFile.write(tPlayer,16);
									outFile.close();
								}
							}
						}
						...
						tBitStream.Write(tPvpAccess);
						tBitStream.Write(tPlayer);
						
						OwnerToken tToken = getTokenByName(tPlayer);
						SystemAddress clientAdd[MAX_CLIENTS];
						server->Send(&tBitStream, HIGH_PRIORITY, RELIABLE_SEQUENCED, 1, clientAdd[tToken-1], false);

						server->Send(&tBitStream, HIGH_PRIORITY, RELIABLE_SEQUENCED, 1, p->systemAddress, false);

						if(showTraffic)
							if(tPvpAccess)printf("%s load char success.\n",tUsername);
							else printf("%s pvp failed.\n",tUsername);
					}
					break;
