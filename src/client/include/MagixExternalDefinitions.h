...
// MagixHandler.h (Этот комментарий можно оставить или убрать)
#include "GameConfig.h"

#include "MagixCrypto.h"
...
// ... (остальной код)
// ...
// В самом конце файла должен быть только ОДИН #endif
...
public:
	void saveHomePoint(const String& mapName, const Ogre::Vector3& position);
	void loadHomePoint(String& mapName, Ogre::Vector3& position);
	bool isAdmin;
	bool isMod;
	bool isModOn;
	...
	void loadSettings(bool &isTablet, Real &mouseSensitivity, Real &musicVol, Real &guiVol, Real &soundVol, bool &isShadowsOn, bool &isBloom, String &username, bool &generalCharname, bool &localUsername, bool &channelBlink, bool &runMode, bool &doubleJumpOn, bool &autoLipSync, bool &useWindowsCursor, bool &Language)
	{
		long tSize = 0;
		char *tBuffer;
		String tData="";
		...
		inFile2.close();
		std::ifstream inFile3("lang.cfg",std::ios::binary);
		char lng[3];
		char* lang;
		if(!inFile3.good())
		{
		...
		{
			inFile3.getline(lng,3);
			lang = lng;
			inFile3.close();
		}
		if(lang=="en") Language==true;
		else Language==false;
	}
	...
	const String XOR7(const String &input, unsigned long *checksum=0, bool preChecksum=false)
	{
		...
		for(int i=0;i<(int)input.length();i++)
		{
			...
			tC ^= XORKEY;//67*(i%3)+9*(i%5)+9; // 7*(i%7+1)
...
}; // <-- Обязательно должна быть } для функции и }; для класса
