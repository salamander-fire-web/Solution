...
public:
	...
	void initialize(MagixExternalDefinitions *def,MagixGameStateManager *gameStateMgr,MagixCritterManager *critterMgr,MagixUnitManager *unitMgr,MagixNetworkManager *networkMgr,MagixWorld *world,MagixChatManager *chatMgr,MagixSkyManager *skyMgr, MagixLanguageManager *lang)
	{
		mDef = def;
	...
	void updateCritters(const FrameEvent &evt)
	{
		...
		for(int i=0;i<(int)tKList.size();i++)
		{
			...
			if(tKList[i]->getIsPet())
			{
				...
				mChatManager->message(mLang->WoundPet());
			}
			else
			{
