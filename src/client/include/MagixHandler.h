...
public:
	MagixHandler()
	{
		...
		mLang = new MagixLanguageManager();
	...
	void initialize(SceneManager *sceneMgr, RenderWindow *window)
	{
		...
		mNetworkManager->initialize(mDef,mGameStateManager,mItemManager,mCritterManager,mUnitManager,mWorld,mSkyManager,mGui->getChatManager(),mGui->getAlertBox(),mGui->getCharScreenManager(), mLang);
		mOwnershipManager->initialize(mDef,mGameStateManager,mCritterManager,mUnitManager,mNetworkManager,mWorld,mGui->getChatManager(),mSkyManager, mLang);
		mGui->initialize(sceneMgr, window, mDef, mGameStateManager, mItemManager, mCritterManager, mUnitManager, mNetworkManager, mCamera, mSoundManager, mEffectsManager, mCollisionManager, mLang);
	...
	bool update(const FrameEvent &evt)
	{
		...
		switch(mGameStateManager->getGameState())
		{
			...
			case GAMESTATE_STARTCONNECTION:
				...
				mGui->showBackdropOverlay(true,true,mLang->Loading());
	...
	void updateCommand()
	{
		...
		if(tCommand==COMMAND_SETHOME)
		{
			...
			mGui->getChatManager()->message(mLang->SetToRecovered());
		}
		else if(tCommand==COMMAND_GOHOME)
		{
			...
			//Same map & channel
			if (mWorld->getWorldName() == "Savannah")
			{
			getMagixGUI()->getAlertBox()->showAlert("You cannot go home yet.",0.5,0.5,3);
			}
			else if(tMap == mWorld->getWorldName() && tDimension == mGameStateManager->getDimension())
			{
				getPlayer()->setPosition(tPoint.x,200,tPoint.y);
		...
		{
			// Исправлено: 1) Убран MAP_DEFAULT; 2) Убран лишний 0
			mDef->saveHomePoint(Ogre::Vector3(Vector2(MAP_SPAWN).x, 200, Vector2(MAP_SPAWN).y));
			mGui->getChatManager()->message(mLang->ResHome());
		}
		else if(tCommand==COMMAND_POSITION)
		{
			mGui->getChatManager()->message(StringConverter::toString(getPlayer()->getPosition())+(";1 1 1;0 "+StringConverter::toString(getPlayer()->getYaw())+" 0"));
		}
	...
	void updateCharScreen(const FrameEvent &evt)
	{
		...
		if(mGui->getCharScreenManager()->popDoReequip())
			for(int i=1;i<=MAX_EQUIP;i++)
				...
				{
					String tEquip = mUnitManager->unequipItem(getPlayer(), i);
