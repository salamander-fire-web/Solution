...
public:
	...
	void initialize(MagixExternalDefinitions *def, MagixGameStateManager *gamestateMgr, MagixItemManager *itemMgr, MagixCritterManager *critterMgr, MagixUnitManager *unitMgr, MagixWorld *world, MagixSkyManager *skyMgr, MagixChatManager *chatMgr, MagixAlertBox *alertBox, MagixCharScreenManager *charscreenMgr, MagixLanguageManager *lang)
	{
		mDef = def;
	...
	bool connect()
	{
		if(serverAdd.size()==0)
		{
			mAlertBox->showAlert(mLang->NoSrv(),0.5,0.5,10);
		...
		if(tLocalIP!="")
		{
			...
			mChatManager->message(mLang->Connecte()+tLocalIP);
		}
		else
		{
			...
			mChatManager->message(mLang->Connecte()+StringConverter::toString(serverAdd[tServerID].first+1));
		}
	...
	void connectMainServer()
	{
		...
		{
			String tMessage = mLang->Banned();
			if(tNumDays>0)
			{
				tMessage += " (" + StringConverter::toString(tNumDays);
				tMessage += tNumDays>1?mLang->DaysLeft():mLang->DasLeft();
			}
	...
	void update(const FrameEvent evt)
	{
		...
		while(packet && tCont)
		{
			...
			case ID_CONNECTION_REQUEST_ACCEPTED:
				...
				{
					mChatManager->message(mLang->Connected());
			...
			case ID_CONNECTION_ATTEMPT_FAILED:
				mAlertBox->showAlert(mLang->OurFail(),0.5,0.5,10);
				if(isMainServerConnection)
			...
			case ID_INVALID_PASSWORD:
				mAlertBox->showAlert(mLang->PleaseLatest(),0.5,0.5,10);
				if(isMainServerConnection)
					networkMessage = NETWORK_CONNECTIONFAILED;
				else
					mGameStateManager->setGameState(GAMESTATE_RETURNSTARTSCREEN);
			...
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				mAlertBox->showAlert(mLang->Unavail(),0.5,0.5,10);
				if(isMainServerConnection)
					networkMessage = NETWORK_CONNECTIONFAILED;
				else
					mGameStateManager->setGameState(GAMESTATE_RETURNSTARTSCREEN);
			...
			case ID_CONNECTION_BANNED:
				mAlertBox->showAlert(mLang->Banned(),0.5,0.5,10);
				disconnect();
				tCont = false;
				break;
			case ID_DISCONNECTION_NOTIFICATION:
				if(isMainServerConnection)
				{
					mAlertBox->showAlert(mLang->YouDisconnected(),0.5,0.5,10);
				...
				else
				{
					...
					mAlertBox->showAlert(mLang->Reconecting(),0.5,0.5,2.5);
					mChatManager->message(mLang->Disconection()+StringConverter::toString(tServerID+1)+".");
					disconnect();
					mItemManager->reset();
			...
			case ID_CONNECTION_LOST:
				if(isMainServerConnection)
				{
					mAlertBox->showAlert(mLang->ConnectionLostS(),0.5,0.5,10);
					networkMessage = NETWORK_LOSTCONNECTION;
				}
				else
				{
					...
					mAlertBox->showAlert(mLang->ConnectionLostR(),0.5,0.5,2.5);
					mChatManager->message(mLang->ConnectionLost()+StringConverter::toString(tServerID+1)+mLang->Reconnecte());
					disconnect();
					mItemManager->reset();
					mCritterManager->reset();
					mUnitManager->reset(false,false);
			...
			case ID_PVP: processPvp(packet); break;
	...
	void sendChat(const String &caption, const unsigned char &type, const String &pmTarget="", bool autoLipSync=false)
	{
		...
		if(type==CHAT_PRIVATE &&pmTarget.length()>0)
		{
			...
			{
				mChatManager->message(mLang->CannotSend());
				return;
			}
			else
			{
				...
				mChatManager->message(mLang->SendTo() + pmTarget + ".");
			}
		}
	...
	void processNewPlayer(Packet *p)
	{
		...
		String tAddMessage = "";
		if(tUnit->isFriend)tAddMessage = mLang->IsFriend();
		if(tUnit->isBlocked)
		{
			tAddMessage = mLang->IsBlocked();
			tUnit->isHidden = true;
		...
		mChatManager->message(tAddName + tAddMessage + mLang->HasArrived(),(mUnitManager->isPartyMember(tUnit)?0:(mChatManager->getChannel()==0?CHAT_LOCAL:CHAT_GENERAL)));
		if(tBadData)mChatManager->message(mLang->ErrNotice() + tAddName + mLang->SentErrors());
	...
	void processPlayerDisconnected(Packet *p)
	{
		...
		mChatManager->message(tName + (tIsProper?mLang->Disconnected():mLang->TimedOut()),(mUnitManager->isPartyMember(tUnit)?0:(mChatManager->getChannel()==0?CHAT_LOCAL:CHAT_GENERAL)));
	...
	void processMapChange(Packet *p)
	{
		...
		const String tName = (tUnit->getUser()==""?tUnit->getName():tUnit->getUser());
		mChatManager->message(tName + mLang->HasLeft(),(mUnitManager->isPartyMember(tUnit)?0:(mChatManager->getChannel()==0?CHAT_LOCAL:CHAT_GENERAL)));
		mUnitManager->deleteUnit(tUnit);
	}
	...
	void sendValidateName()
	{
		...
		for(int i=0;i<MAX_EQUIP;i++)
		{
			...
			{
				mUnitManager->getPlayer()->setPet("Great Hawk");
				(mUnitManager->getPlayer()->getPet()!="Great Hawk");
				mChatManager->message(mLang->Egg1());
			}
			else if(mUnitManager->getPlayer()->getEquip(i)=="Egg2")
			{
				mUnitManager->getPlayer()->setPet("Overgrown Gazelle");
				(mUnitManager->getPlayer()->getPet()!="Overgrown Gazelle");
				mChatManager->message(mLang->Egg2());
			}
			else if(mUnitManager->getPlayer()->getEquip(i)=="Egg3")
			{
				mUnitManager->getPlayer()->setPet("Giant Manta Ray");
				(mUnitManager->getPlayer()->getPet()!="Giant Manta Ray");
				mChatManager->message(mLang->Egg3());
			}
			else if(mUnitManager->getPlayer()->getEquip(i)=="Egg4")
			{
				mUnitManager->getPlayer()->setPet("Zombie Dragon");
				(mUnitManager->getPlayer()->getPet()!="Zombie Dragon");
				mChatManager->message(mLang->Egg4());
			}
			else if(mUnitManager->getPlayer()->getEquip(i)=="Egg5")
			{
				mUnitManager->getPlayer()->setPet("Sea Serpent");
				(mUnitManager->getPlayer()->getPet()!="Sea Serpent");
				mChatManager->message(mLang->Egg5());
			}
		}
	...
	void processPlayerData(Packet *p)
	{
		...
		else
		{
			...
			if(mUnitManager->removePartyMember(tName))mChatManager->message(mLang->PMember()+String(tName)+mLang->Disconnected(),CHAT_PARTY);
		}
	}
	...
	void processBio(Packet *p)
	{
		...
		else
		{
			...
			if(targetBioUpdated=="")targetBioUpdated = mLang->None();
		}
	}
	...
	void sendPvp(const String &arg1, const String &arg2)
	{
		RakNet::BitStream tBitStream;
		mChatManager->message("эээ1");

		tBitStream.Write(MessageID(ID_PVP));
		stringCompressor->EncodeString(arg1.c_str(),16,&tBitStream);
		stringCompressor->EncodeString(arg2.c_str(),16,&tBitStream);
		mChatManager->message("эээ2");

		mPeer->Send(&tBitStream, MEDIUM_PRIORITY, UNRELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS, true);
		mChatManager->message("эээ3");
	}
	void processPvp(Packet *p)
	{
		if(!isMainServerConnection)return;
		mChatManager->message("эээ4");
		RakNet::BitStream tBitStream(p->data, p->length, false);
		MessageID tID;
		bool tAccess;
		char tName[16] = "";
		...
		tBitStream.Read(tID);
		tBitStream.Read(tAccess);
		tBitStream.Read(tName);
		if(tAccess)
		{
			mUnitManager->setMapChange(true,"Waterfall",true,Vector3(0,0,0));
		}
	}
	...
	bool processKick(Packet *p)
	{
		...
		{
			disconnect();
			mAlertBox->showAlert(mLang->KickedAdmin(),0.5,0.5,10);
			mChatManager->message(mLang->KickedAdmin());
	...
	void processItemPickup(Packet *p)
	{
		...
		if(tSuccess)
		{
			...
			if(tSlot!=-1)
			{
				mChatManager->message(mLang->Equipped()+mDef->getItemName(tMeshName)+".");
	...
	bool processForceLogout(Packet *p)
	{
		...
		{
			disconnect();
			mAlertBox->showAlert(mLang->DisByUser(),0.5,0.5,10);
			mChatManager->message(mLang->DisByAnother());
			return false;
		}
		return true;
	}
	...
	void processInfo(Packet *p)
	{
		...
		else if(type==INFO_PARTYACCEPTED)
		{
			...
			for(int i=0;i<MAX_PARTYMEMBERS;i++)
			{
				...
				mChatManager->message(String(tMemberName)+mLang->HasJoinedP(),CHAT_PARTY);
		...
		else if(type==INFO_PARTYLEAVE)
		{
			...
			mChatManager->message(tMember.second+mLang->HasLeftP(),CHAT_PARTY);
			return;
		}
		...
		{
			mChatManager->message(tUnit->getUser()+mLang->AlreadyInP(),CHAT_PARTY);
		}
	}
	...
	void sendPartyLeave()
	{
		...
		mUnitManager->clearPartyMembers();
		mChatManager->message(mLang->YouLeftP());
	}
	...
	void processCritterInfo(Packet *p)
	{
		...
		if(tIsTamed)
		{
			...
			if(tImTheOwner)
			{
				...
				mChatManager->message(mLang->YouHaveTamed()+(tVowel?String(mLang->An()):String(mLang->A()))+tType+".");
				if(mUnitManager->getPlayer()->getPet()!="")mChatManager->message(mLang->HaveYouTamed());
	...
	void processPlayerInfo(Packet *p)
	{
		...
		if(tIsWounded)
		{
			...
			if(mUnitManager->isPartyMember(unit))mChatManager->message(unit->getName()+mLang->IsWonded(),CHAT_PARTY);
		}
		//Unwounded
		...
		if(tIsUnwounded)
		{
			...
			if(mUnitManager->isPartyMember(unit))mChatManager->message(unit->getName()+mLang->RecoverWound(),CHAT_PARTY);
		}
	}
	...
	void processItemStash(Packet *p)
	{
		...
		if(tIsStash)
		{
			...
			else
			{
				...
				mChatManager->message(mLang->Moved()+mDef->getItemName(tEquip)+mLang->IntoStash());
			}
		}
		//Unstash
		else
		{
			...
			{
				mChatManager->message(mLang->Equipped()+mDef->getItemName(tEquip)+".");
	...
	void processFindPlayer(Packet *p)
	{
		...
		if(tOnline)
		{
			...
			mChatManager->message(String(tName)+"["+StringConverter::toString(tToken)+"]"+mLang->IsIn()+String(tMap)+mLang->Dimension()+StringConverter::toString(tDimension)+".");
		}
		else mChatManager->message(String(tName)+mLang->IstOnline());
	}
	...
	void processMaintenance(Packet *p)
	{
		...
		mChatManager->message(mLang->SrvRestartingIn()+StringConverter::toString((int)tCount)+mLang->PleaseLogOut());
	}
