...
#define COMMAND_PVP "/pvp"
// KITO
#define COMMAND_SPAWNITEM "/spoonitem "
...
public:
	...
	MagixChatManager()
	{
		...
		for (int i = 0; i < MAX_CHANNELS; i++) hasNewLine[i] = false;
	}
	~MagixChatManager()
	{
	}
	...
	void processInput(String &caption, unsigned char &type, String &param)
	{
		...
		//PVP
		if(StringUtil::startsWith(caption,COMMAND_PVP))
		{
			caption.erase(0,7);
			type = CHAT_COMMAND;
			param = COMMAND_PVP;
	...
	const UTFString prefixChatLine(const UTFString &caption, const String &sayer, const unsigned char &type, bool isPostfixLength=false)
	{
		...
		{
			if(type==CHAT_ADMIN)tLine = tLine + "(" USER_ADMIN_TEXT ")";
			else if(type==CHAT_MOD)tLine = tLine + "(" USER_MOD_TEXT ")";
			if(type==CHAT_ACTION||type==CHAT_PARTYACTION||type==CHAT_LOCALACTION)tLine = tLine + sayer + " ";
			else if(type==CHAT_PRIVATE)tLine = tLine + "((" + sayer + ")) ";
			else if(type==CHAT_EVENT||type==CHAT_LOCALEVENT||type==CHAT_PARTYEVENT)tLine = tLine + "[" + sayer + " ";
			else tLine = tLine + "<" + sayer + "> ";
		}
		//Caption
		tLine = tLine + caption;
		//Postfix
		...
		{
			if(type==CHAT_EVENT||type==CHAT_LOCALEVENT||type==CHAT_PARTYEVENT)tLine = tLine + "]";
		}
	...
	const String processChatString(const UTFString &caption, const String &sayer, const unsigned char &type, const Real &boxWidth, const Real &charHeight)
	{
		...
		{
			//size caption
			const Font *pFont = dynamic_cast<Ogre::Font*>(Ogre::FontManager::getSingleton().getByName("orbitron-medium").getPointer());
