...
public:
	...
	void initialize(MagixExternalDefinitions *def, MagixEffectsManager *effectsMgr, MagixAlertBox *alertBox, MagixUnit *unit, SceneManager *sceneMgr, OverlayElement *name, OverlayElement *backdrop, MagixLanguageManager *lang)
	{
		mDef = def;
		mLang = lang;
		...
		if(button==mDeleteButton)
		{
			...
			if(page==PAGE_EDIT)
			{
				if(deleteConfirmation=="")
				{
					...
					mName->show();
					mAlertBox->showAlert(mLang->EntCharName(),mName->getLeft()+0.25,mName->getTop()+mName->getHeight()+0.05);
					return 0;
				}
				if(deleteConfirmation!=getName())
				{
					mAlertBox->showAlert(mLang->CharNMiss(),mName->getLeft()+0.25,mName->getTop()+mName->getHeight()+0.05);
					return 0;
				}
	...
	{
		const String tPart[MAX_COLOURS] = {mLang->Pelt(),mLang->Underfur(),mLang->Mane(),mLang->Nose(),mLang->AboveEyes(),mLang->BelowEyes(),mLang->Ears(),mLang->Tailtip(),mLang->Eyes(),mLang->Wings(),mLang->Markings()};
		return tPart[iID];
	}
	...
	void refreshPage()
	{
		...
		if(page==PAGE_SELECT)
		{
			...
			if(getName()!="")
			{
				...
				mNextButtonText->setCaption(mLang->Start());
				...
				mDeleteButtonText->setCaption(mLang->Edit());
			}
			else
			{
				if(charID==numChars)
				{
					if(!mNextButton->isVisible())mNextButton->show();
					mNextButtonText->setCaption(mLang->New());
		...
		if(page==PAGE_CUSTOM1)
		{
			...
			mNextButtonText->setCaption(mLang->Next());
			if(mDeleteButton->isVisible())mDeleteButton->hide();
			return;
		}
		//Mesh page 2
		...
		if(page==PAGE_CUSTOM5)
		{
			...
			mBackButton->setPosition(0.05,0.6);
			mNextButton->setPosition(0.3,0.6);
			mNextButtonText->setCaption(mLang->Finish());
			return;
		}
		//Preset page
		...
		if(page==PAGE_EDIT)
		{
			...
			if(!mText[0]->isVisible())mText[0]->show();
			mText[0]->setPosition(0.25,0.05);
			mText[0]->setCaption(mLang->EditCharacter());
			for(int i=0;i<MAX_CHARSCREENTEXT;i++)
			{
				if(mLButton[i]->isVisible())mLButton[i]->hide();
				if(mRButton[i]->isVisible())mRButton[i]->hide();
			...
			mBackButton->setPosition(0.05,0.6);
			mNextButton->setPosition(0.18,0.30);
			mNextButtonText->setCaption(mLang->Edit());
			if(!mDeleteButton->isVisible())mDeleteButton->show();
			mDeleteButton->setPosition(0.18,0.45);
			mDeleteButtonText->setCaption(mLang->Delete());
			return;
		}
		//Preset page (edit)
		if(page==PAGE_PRESET1)
	...
	const unsigned short nextPage()
	{
		...
		{
			String tAlert = mLang->MustNotInc();
			if(getName().length()==0)tAlert = mLang->EntName();
			else if(getName()[0]==' ')tAlert = mLang->MustNotSpace();
